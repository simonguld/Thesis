...
dt = 0.000005
As =  [5,16] #list of amplitudes to check 
Ts = 1./(np.array([0.1,1.0,25.])) * (1/dt)#[1000,5000,10000]#[500,1000,1500,2000] #[500,1000,1500,2000] #list of time periods to check 
...
if (shake):				
	for i, j,trial in itertools.product(np.arange(len(As)),np.arange(len(Ts)),np.arange(trials)): #iterate through amplitude/time period combinations 
		...
		mainDirName = "shakeSweep/ramps_0/A_%f_T_%f/trial_%d/"%(A,T,trial) #Where simulation-specific files go 
		...
		#if (Rve.nSteps <= 550): continue #skip unfinished simulations
		visDir = "shakeSweep/plots/ramps_0/A_%f_F_%f_trial_%d/"%(A,1./(dt*T),trial)  #directory for visualizing this RVE 
		...
		Rve.plot(Rve.rightWallPos,Rve.rightWallPos,figDir,vidName,1,1) #plot the simulation
		...
		PFs = helper.addValue(PFs,A,(1./T)*(1/dt),PF) 
