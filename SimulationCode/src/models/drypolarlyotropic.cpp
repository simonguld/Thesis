...
void DryPolarLyotropic::UpdatePolarQuantitiesAtNode(unsigned k)
{
  ...
  const double hx = CC*term*4*ps*px + Kp*laplacian(Px, d, sD)
	  	  + Kn*( 1.*ps*laplacian(Px, d, sD)
         	  	+2.*px*(dxpx*dxpx+dypx*dypx-dxpy*dxpy-dypy*dypy)
         	  	+4.*py*(dxpx*dxpy+dypx*dypy));
  const double hy = CC*term*4*ps*py + Kp*laplacian(Py, d, sD)
	  	  + Kn*( 1.*ps*laplacian(Py, d, sD)
         	        +2.*py*(dxpy*dxpy+dypy*dypy-dxpx*dxpx-dypx*dypx)
         		+4.*px*(dxpx*dxpy+dypx*dypy));

  const double mu = (surftension_on? Aphi*p*(p-1.)*(p+1.) + CC*term - Kphi*del2p : Aphi*(p-1.) + 0.0*CC*term - Kphi*del2p);// 
  ...
  const double sigmaxx = -(p > 1.0 ? 1 : 0)*.5*zeta*(px*px-py*py) + ( ( backflow_on? -2.*xi*px*hx-xi*py*hy : 0) ) + sigmaB + sigmaF;
  const double sigmayy = -(p > 1.0 ? 1 : 0)*.5*zeta*(py*py-px*px) + ( ( backflow_on? -2.*xi*py*hy-xi*px*hx : 0) ) + sigmaB - sigmaF;
  const double sigmaxy = -(p > 1.0 ? 1 : 0)*zeta*px*py + ( ( backflow_on? .5*(1.-xi)*px*hy - .5*(1.+xi)*py*hx :0) ) + sigmaS;
  const double sigmayx = -(p > 1.0 ? 1 : 0)*zeta*py*px + ( ( backflow_on? .5*(1.-xi)*py*hx - .5*(1.+xi)*px*hy :0) ) + sigmaS;
...
void DryPolarLyotropic::UpdateFluidQuantitiesAtNode(unsigned k)
{
  ...
  // compute velocities
  const double vx = (FFx[k] + (p > 1.0 ? 1 : 0)*alpha*px - beta*hx)/friction;
  const double vy = (FFy[k] + (p > 1.0 ? 1 : 0)*alpha*py - beta*hy)/friction;
...
{  
  ...
  const double Dx = hx/gamma 
	  	    - (vx + Vp*px)*dxpx - (vy + Vp*py)*dypx
                    + (xi*(dxux*px + .5*(dyux+dxuy)*py) + .5*(dyux-dxuy)*py )
		    + beta*vx;
  const double Dy = hy/gamma 
	  	    - (vx + Vp*px)*dxpy - (vy + Vp*py)*dypy
                    + (xi*(dyuy*py + .5*(dxuy+dyux)*px) + .5*(dxuy-dyux)*px )
		    + beta*vy;
