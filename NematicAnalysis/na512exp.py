...
def main():
    ...
    if 'fig2' in do_plotting_for or 'all' in do_plotting_for:
        ...
        ax2.xaxis.set_minor_locator(ticker.MultipleLocator(2e-3))   
        ax2.yaxis.set_minor_locator(ticker.MultipleLocator(1))     
        ax2.set_xlim([0, 0.052])
        ax2.set_ylim(ymin=0, ymax=11)
        ax2.set_xlabel(r'Activity ($\zeta$)')
        ax2.set_ylabel(r'Active susceptibility ($\overline{\chi_a})$')
        ax2.vlines(x=0.022, ymin = -1.2, ymax=1.8, linestyle='--', color='k', lw = 1, zorder=-10)
    ...
    if 'fig3' in do_plotting_for or 'all' in do_plotting_for:
        ...
        ax3.set_xlim([0, 0.052])
        ax3.set_ylim(ymin=-1.1, ymax=.2)
        ax3.set_xlabel(r'Activity ($\zeta$)')
        ax3.set_ylabel(r'Hyperuniformity exponent ($\gamma$)')
        ax3.vlines(x=0.022, ymin = -1.2, ymax=-.56, linestyle='--', color='k', lw = 1)
