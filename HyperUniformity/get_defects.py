...
# get fluctuation stuff working
# make equilibrium estimation dynamic
...
def main():
    ...
    for i, dir in enumerate(dirs):
        ...
        if 1:
            ...
            def_density_arr[i] = dens_defects

            # Plot logarithm of density fluctuations against windows sizes for given activity
            fig, ax = plt.subplots()
        ...
        if save_density_plots:
            ...
            fig, ax = plt.subplots()
            ax.plot(dens_defects, '.-', label = 'Total defects')
            ax.set_xlabel('Frame')
