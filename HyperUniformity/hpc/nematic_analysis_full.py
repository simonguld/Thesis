...
# Set upper and lower bounds to run analysis on
act_min = 0.0
act_max = 0.12
exp_max = 2
...
if run_locally:
    ...
    folder_path = "/lustre/astro/kpr279/nematic_simulation512_LL0.05"
    output_path = '/lustre/astro/kpr279/nematic_analysis_512_LL0.05_test'
...
def get_defect_list(archive, LX, LY, idx_first_frame=0, verbose=False):
    ...
    if verbose:
        ...
        print('Time to get defect list: %.2f s' % t_end, flush = True)
...
def calc_density_fluctuations(points_arr, window_sizes, boundaries = None, N_center_points=None, Ndof=1, dist_to_boundaries=None, normalize=False):
    ...
    if N_center_points > Npoints_within_boundaries:
        print(f"Warning: N_center_points is larger than the number of points within the boundaries.\
               Using all {Npoints_within_boundaries} points within boundaries instead.", flush = True)
    ...
    if N_center_points == 0:
        print(f"No points within boundaries. Returning NaNs", flush = True)
...
def main():
    ...
    for dir in ignore_dirs_in_folder_path:
        dirs_all.remove(dir)
    ...
    for dir in dirs_all:
        if os.path.isdir(os.path.join(folder_path, dir)):
            zeta = float(dir.split('_')[-1])
            if act_min <= zeta <= act_max:
                activity_list.append(float(zeta))
                dirs_act.append(dir)

    dirs_act = [x for _, x in sorted(zip(activity_list, dirs_act))]
    activity_list = sorted(activity_list)

    print(f"Activity list: {activity_list}", flush = True)
    print(f"Dirs: {dirs_act}", flush = True)

    for act, dir_act in zip(activity_list, dirs_act):
        act_path = os.path.join(output_path, f'act{act}')
        if not os.path.isdir(act_path):
            os.mkdir(act_path)

        completed_path = os.path.join(act_path, 'analysis_completed.txt')
        model_params_path = os.path.join(act_path, 'model_params.pkl')

        if os.path.isfile(completed_path):
            msg = f"Analysis for activity {act} has already been analyzed. Continuing to next activity"
        ...
        for dir in os.listdir(os.path.join(folder_path, dir_act)):
                num_exp = int(dir.split('_')[-1])
                if num_exp <= exp_max:
                    exp_list.append(num_exp)
                    dirs_exp.append(dir)
        dirs_exp = [x for _, x in sorted(zip(exp_list, dirs_exp))]
        exp_list = sorted(exp_list)
        
        print(f"Experiment list: {exp_list}", flush = True)
        print(f"Dirs exp: {dirs_exp}", flush = True)

        for exp, dir_exp in zip(exp_list, dirs_exp):
            archive_path = os.path.join(folder_path, dir_act, dir_exp)
            av_defects_path = os.path.join(act_path, f'Ndefects_act{act}_exp{exp}.txt')
            var_counts_path = os.path.join(act_path, f'count_fluctuations_act{act}_exp{exp}.txt')
            av_counts_path = os.path.join(act_path, f'av_counts_act{act}_exp{exp}.txt')
            window_sizes_path = os.path.join(act_path, f'window_sizes.txt')
            ...
            if os.path.isfile(av_defects_path) and os.path.isfile(var_counts_path) and os.path.isfile(av_counts_path):
                msg = f"Analysis for experiment {exp} and activity {act} has already been done. Continuing to next activity"
                print(msg, flush = True)
                continue
            ...
            if development_mode:
                ...
                if not converged:
                    print("Experiment ", exp, " act. ", act, " did not converge", flush = True)
        ...
        # save txt to indicate act analysis done
        t_end = np.round(time.time() - t_start, 2)
        msg = f"Time to analyze activity {act}: {t_end} s \n\n"
        print(msg, flush = True)
        gen_status_txt(msg, completed_path)
